      ...
      <div className="relative z-10 container mx-auto px-3 sm:px-6">
        ...
        <div className="grid grid-cols-3 gap-2 sm:gap-6 max-w-6xl mx-auto">
          {projects.map((project, index) => (
            <div
              key={project.title}
              className="glass-frosted rounded-xl sm:rounded-2xl overflow-hidden group hover:scale-[1.02] transition-all duration-300 animate-fade-up cursor-pointer"
              style={{ animationDelay: `${index * 0.1}s` }}
            >
              {/* Image Placeholder */}
              <div className="aspect-square sm:aspect-[4/3] relative glass-heavy overflow-hidden">
                <div className="absolute inset-0 flex items-center justify-center">
                  <div className="w-8 sm:w-16 h-8 sm:h-16 rounded-full glass-icon flex items-center justify-center group-hover:scale-110 transition-transform">
                    <ArrowUpRight className="w-3 sm:w-6 h-3 sm:h-6 text-accent" />
                  </div>
                </div>
                {/* Decorative gradient */}
              ...
              <div className="p-2 sm:p-6">
                <div className="flex items-center justify-between mb-1 sm:mb-3">
                  ...
                  <span className="text-[8px] sm:text-xs font-medium text-accent glass-icon px-1.5 sm:px-3 py-0.5 sm:py-1 rounded-full">
                    {project.category}
                  </span>
                </div>
                <h3 className="font-display text-xs sm:text-xl font-bold text-foreground mb-1 sm:mb-2 group-hover:text-accent transition-colors">
                  {project.title}
                </h3>
